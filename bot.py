...
async def callback_handler(client, callback_query):
    ...
    if data.startswith("movie:"):
        ...
        caption = (
            f"<b>{details['title']}</b>\n"
            f"{(details.get('language') or '').upper()} · "
            f"{details.get('year')} ⭐ {details.get('rating')}"
        )
    ...
    if data.startswith("copy:"):
        admin_ids = [
            int(x) for x in (config.ADMIN or "").split(",")
            if x.strip().isdigit()
        ]

        if callback_query.from_user.id not in admin_ids:
            await callback_query.answer(
                templates.NOT_ADMIN_BREACH,
                show_alert=True
            )
            return
...
@app.on_message(
    ...
    & filters.user(int(config.ADMIN)) if config.ADMIN.isdigit() else filters.user([])
)
